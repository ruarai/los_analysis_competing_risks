...
onset_fits <- tar_read(surv_onset_to_ward)
ward_fits <- tar_read(surv_ward_to_next)
ICU_fits <- tar_read(surv_ICU_to_next)
postICU_fits <- tar_read(surv_postICU_to_next)


pathway_probabilites <- tar_read(pathway_probabilities)
...
pathway_results_tbl <- pathway_probabilites %>%
