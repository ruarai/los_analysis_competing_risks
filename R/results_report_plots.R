...
make_reporting_plots_1 <- function(
  all_aj,
  results_dir
) {
  require(cowplot)
  ...
  plot_group <- function(
    i_coding, i_age_type, i_age_class,
  ...
  plot_grid(
    ...
    add_sub(
      ...
      "Time since ward admission (days)", size = 11, vjust = 0
    ),
    add_sub(
      plot_grid(
    ...
    add_sub(
      ...
      "Time since ICU admission (days)", size = 11, vjust = 0
    ),
