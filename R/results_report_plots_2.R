...
make_reporting_plots_2 <- function(all_means,
                                   results_dir) {
  all_means <- remove_bad_fits(all_means)



  plot_group <- function(i_coding, i_age_type, y_max,
    ...
    p <- ggplot(plot_data) +
      geom_linerange(aes(
        x = age_class, ymin = mean_lower, ymax = mean_upper,
