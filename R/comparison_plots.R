...
# results_dir <- "results/comp_omi_HNE_2022-01-04_omi_mix_2022-01-04"
# results_filt <- c("omi_mix", "omi_HNE")

results_dir <- "results/comp_omi_mix_2022-01-04_delta_2021-11-15"
results_filt <- c("omi_mix", "delta")

datasets <- list(
  "omi_mix" = read_rds("results/NSW_omi_mix_2022-01-04/surv_plots_data.rds"),
  "delta" = read_rds("results/NSW_delta_2021-11-25/surv_plots_data.rds"),
  "omi_HNE" = read_rds("results/NSW_omi_HNE_2022-01-04/surv_plots_data.rds")
)
...
p_list <- pmap(plot_meta, function(i_comp, i_age_type) {
  ...
  filter_comp <- . %>% filter(coding == i_comp, age_type == i_age_type, source %in% results_filt)
...
est_labels <- c(
  ...
  "omi_HNE" = "Omicron (Hunter New England LHD)"
)
...
cowplot::plot_grid(
  cowplot::plot_grid(plotlist = p_list[1:4], ncol = 1),
  plot_legend,
  rel_heights = c(20, 1), ncol = 1
)
