...
make_results_tables <- function(onset_fits, ward_fits, ICU_fits, postICU_fits,
  ...
  pathway_results_tbl <- pathway_probabilities %>%
    mutate(
      est = str_c(round3(prop_pathway), " (", round3(lower_95), ", ", round3(upper_95), ")"),
  ...
  fits_tbl <- fits_final %>%
    mutate(
      mean_ci = make_ci(mean_est, mean_lcl, mean_ucl),
